...
describe('LaunchGenerator', () => {
  it('single project with auto-generated config', async () => {
    ...
    try {
      ...
      assertExists(await launchConfigFile.isFile());

      const launchConfig = await launchConfigFile.readJson() as { launch: { groups: Group[] } };
      assertEquals(launchConfig.launch.groups.length, 2);

      // Check that launch.json was created and has generated configurations
      const launchJsonFile = new FileSpec(root, '.vscode', 'launch.json');
      assertExists(await launchJsonFile.isFile());
      ...
      assertEquals(generatedConfigs.length, 3);
    } finally {
      await Deno.remove(root.path, { recursive: true });
    }
  });

  it('monorepo with workspace configurations', async () => {
    const root = await createTempProject();
  ...
  it('custom groups with program and scripts', async () => {
    const root = await createTempProject();
  ...
  it('preserves manual configurations', async () => {
    const root = await createTempProject();
  ...
  it('configuration merging between deno.json and launch.config.json', async () => {
    const root = await createTempProject();
  ...
  it('executable exports auto-generation', async () => {
    const root = await createTempProject();
    ...
    try {
      ...
      await denoJsonFile.writeJson(
        {
          ...
          exports: {
            ...
            'cli': './cli.ts', // Should be included (contains import.meta.main test)
            'cli2': './src/cli.ts', // Should be included (contains import.meta.main test)
            'server': './server.ts', // Should be included (includes hashbang)
            'server2': './src/server.ts', // Should be included (includes hashbang)
            'main': './main.ts', // Should be included (main.ts in root folder)
            'main2': './src/main.ts', // Should be excluded (main.ts in src folder)
          },
        },
        null,
        2,
      ...
      await new FolderSpec(root, 'src').ensureDir();
      const _mod = await new FileSpec(root, 'mod.ts').write('//Module');
      const _cli1 = await new FileSpec(root, 'cli.ts').write('// CLI\nif (import.meta.main) { main(); }');
      const _cli2 = await new FileSpec(root, 'src', 'cli.ts').write('// CLI\nif (import.meta.main) { main(); }');
      const _server1 = await new FileSpec(root, 'server.ts').write('#!deno\n// Server');
      const _server2 = await new FileSpec(root, 'src', 'server.ts').write('#!deno\n// Server');
      await new FileSpec(root, 'main.ts').write('// Main file in root');
      await new FileSpec(root, 'src', 'main.ts').write('// Main file in src');
      ...
      // Should have 2 default groups + 5 from exports
      assertEquals(launchConfig.launch.groups.length, 7);

      const findGroup = (id: string) => launchConfig.launch.groups.find((g: Group) => g.id === id);

      const testGroup = findGroup('test');
      const runGroup = findGroup('run');
      const cliGroup = findGroup('cli');
      const cli2Group = findGroup('cli2');
      const serverGroup = findGroup('server');
      const server2Group = findGroup('server2');
      const mainGroup = findGroup('main');

      assertExists(testGroup);
      assertExists(runGroup);
      assertExists(cliGroup);
      assertExists(cli2Group);
      assertExists(serverGroup);
      assertExists(server2Group);
      assertExists(mainGroup);

      assertEquals(cliGroup.program, './cli.ts');
      assertEquals(cli2Group.program, './src/cli.ts');
      assertEquals(serverGroup.program, './server.ts');
      assertEquals(server2Group.program, './src/server.ts');
      assertEquals(mainGroup.program, './main.ts');
    } finally {
      await Deno.remove(root.path, { recursive: true });
    }
  });

  it('monorepo with root-level workspaces', async () => {
    const root = await createTempProject();
  ...
  it('correctly omits redundant settings', async () => {
    const root = await createTempProject();
  ...
  it('ignores non-runnable exports', async () => {
    const root = await createTempProject();
    ...
    try {
      ...
      // Create deno.json with a non-runnable export
      const denoJsonA = new FileSpec(workspaceA, 'deno.json');
      await denoJsonA.writeJson({
        name: 'workspace-a',
        version: '1.0.0',
      ...
      await Deno.writeTextFile(new FileSpec(workspaceA, 'mod.ts').path, '// Library module');

      // Create a test file to ensure other configurations are still generated
      await Deno.writeTextFile(new FileSpec(workspaceA, 'my.test.ts').path, '// Test file');
      ...
      // Should have a config for the test file, but not for the mod.ts export
      assertEquals(workspaceAConfigs.length, 1);
      assert(workspaceAConfigs[0].name.includes('my.test.ts'));
    } finally {
      await Deno.remove(root.path, { recursive: true });
    }
  });
